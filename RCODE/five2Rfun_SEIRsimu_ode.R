SEIRsimuR <- function(pars, init_settings, num_periods = 2,t=0) {
  ...
  for (i_stage in 1:2) {
    ...
    for (d in stage_start[i_stage]:stage_end[i_stage]) {
      ...
      stage_pars_setings <- c(b =0.6*b_vec[i], r =0.3*r_vec[1]/0.6, rh= ((rh_vec[i]-r_vec[1])/(1-r_vec[1]))*(1-r_vec[1]*0.3/0.6)+r_vec[1]*0.3/0.6,Dq = Dq_vec[dd%/%5.00001+1],Dqs = Dqs_vec[dd%/%5.00001+1], n = flowN_vec[i],bs=0.6*bs_vec[i],bl=0.6*bl_vec[i])
