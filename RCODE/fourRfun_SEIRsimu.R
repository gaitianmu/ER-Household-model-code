...
SEIRsimuR <- function(pars, init_settings, num_periods = 2,t=0) {
  ...
  for (i_stage in 1:2) {
    ...
    for (d in stage_start[i_stage]:stage_end[i_stage]) {
      ...
      bs_vec[2]<-bs_vec[1]*b_vec[2]/b_vec[1]
      
      stage_pars_setings <- c(b = b_vec[i], r = r_vec[1], rh= rh_vec[i],Dq = Dq_vec[dd%/%5.00001+1],Dqs = Dqs_vec[dd%/%5.00001+1], n = flowN_vec[i],bs=bs_vec[i],bl=bl_vec[i])
