SEIRsimu <- function(pars, init_settings, num_periods = 2) {
  ...
  for (i_stage in 1:2) {
    ...
    for (d in stage_start[i_stage]:stage_end[i_stage]) {
      ...
      stage_pars_setings <- c(b =2*b_vec[i_stage], r = r_vec[1], rh= rh_vec[i_stage],Dq = Dq_vec[dd%/%5.00001+1],Dqs = Dqs_vec[dd%/%5.00001+1], n = flowN_vec[i_stage],bs=2*bs_vec[i_stage],bl=2*bl_vec[i_stage])
