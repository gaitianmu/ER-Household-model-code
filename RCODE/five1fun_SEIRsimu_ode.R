SEIRsimu <- function(pars, init_settings, num_periods = 2) {
  ...
  update_func <- function(stage_pars, states_old) {
    ...
    pSS_vec <- c(b * (alpha * (P+PS+PL) +alpha * (A+AS+AL) +alpha * (AA+AAS+AAL)) / N, b *(I+IS+IL) /N,(MS-1)*b * (alpha * (P+PS+PL) +alpha * (A+AS+AL) +alpha * (AA+AAS+AAL)) / N,(MS-1)*b *(I+IS+IL) /N, n /N, 1 - MS*b * (alpha * (P+PS+PL) +alpha * (A+AS+AL) +alpha * (AA+AAS+AAL)) / N-MS*b *(I+IS+IL) /N- n / N)  #(SL+EL+PL+ALe+ALl+ILe+ILl)
  ...
  for (i_stage in 1:2) {
    ...
    for (d in stage_start[i_stage]:stage_end[i_stage]) {
      ...
      stage_pars_setings <- c(b =0.8* b_vec[i_stage], r = 0.3*r_vec[1]/0.8, rh=  ((rh_vec[i_stage]-r_vec[1])/(1-r_vec[1]))*(1-r_vec[1]*0.3/0.8)+r_vec[1]*0.3/0.8,Dq = Dq_vec[dd%/%5.00001+1],Dqs = Dqs_vec[dd%/%5.00001+1], n = flowN_vec[i_stage],bs=0.8*bs_vec[i_stage],bl=0.8*bl_vec[i_stage])
