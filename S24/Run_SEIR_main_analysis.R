...
source(paste0(code_root, "RCODE/fun_SEIRsimu.R"))
source(paste0(code_root, "RCODE/Rfun_SEIRsimu.R"))
source(paste0(code_root, "RCODE/fun_SEIRfitting.R"))
source(paste0(code_root, "RCODE/roinit_cond.R"))
source(paste0(code_root, "RCODE/correlationPlot_modified.R"))
source(paste0(code_root, "RCODE/rofun_SEIRplot.R"))
source(paste0(code_root, "RCODE/fun_Findzero.R"))

##

init_sets_list=get_init_sets_list(alpha = 0.55)
probbase<- read.csv("/home/gaixin/data/roprobability-14.csv", row.names = 1)
prob<-probbase[1:52,]
qd<-prob[8:52]
set.seed(66)
SEIRfitting(init_sets_list, randomize_startValue = T,
            run_id = "S24-random", output_ret = T, skip_MCMC=F)
